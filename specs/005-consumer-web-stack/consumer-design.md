...
- [ ] **A: Scaffold** -- Create the file structure: `versions.tf` (terraform block with cloud block, required_version, required_providers), `providers.tf` (AWS provider with region and default_tags), `variables.tf` (all variables from the Variables table), `outputs.tf` (all outputs from the Outputs table), `locals.tf` (name_prefix, component names, common_tags, wiring computations), `data.tf` (aws_vpc, aws_subnets, aws_caller_identity data sources), `terraform.auto.tfvars.example` (example values)
